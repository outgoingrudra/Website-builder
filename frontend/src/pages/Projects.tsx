      ...
      <div className="flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar">
        <div className="flex items-center gap-2 sm:min-w-90 text-nowrap">
          ...
          <div className="sm:hidden flex-1 flex justify-end">
            ...
            : <XIcon className="s-6 cursor-pointer" onClick={()=> setIsMenuOpen(true)} />
            }
          </div>
        </div>
        ...
        <div className="hidden sm:flex gap-2 bg-gray-950 p-1.5 rounded-md">
          ...
          <TableIcon onClick={()=> setDevice('tablet')} className={`size-6 p-1
        ...
        <div className="flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm">
            <button onClick={saveProject} disabled={isSaving} className="max-sm:hidden bg-gray-800 hover:bg-gray-700 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors border border-gray-700">{isSaving ? <Loader2Icon className="animate-spin" size={16} /> :
              <SaveIcon size={16} />}Save</button>
            <Link target='_blank' to={`/preview/${projectId}`} className="flex items-center gap-2 px-4 py-1 rounded sm:rounded-sm border border-gray-700 hover:border-gray-300 transition-colors"><FullscreenIcon size={16} />Preview</Link>
